...
| 2026-06-18 | **작업 분담**: 맥북 = 문서·맥락·프로젝트 업데이트 / 회사 Windows PC = 실습·검증(TIA 실제 조작) | 회사 PC가 느려 작업이 번거로움. 맥북에 git 저장소를 본인 레포 `wonmotime-1/bsfa-tia`로 연결 |
...
### 2026-06-18 — 맥북 작업환경 구축 + 작업 분담 확정
- 맥북 프로젝트 폴더를 git 저장소로 연결 → 본인 계정 레포 `wonmotime-1/bsfa-tia`(이전 push본을 포크). 로컬=원격 내용 동일 확인, 추가 push 불필요.
- **작업 분담 확정**: **맥북 = 문서·맥락·프로젝트 업데이트**(빠르고 편함) / **회사 Windows PC = 실습·검증**(TIA Portal 실제 조작·라이선스·import). 회사 PC가 느려 번거롭기 때문.
- 함의: TIA 실제 조작이 필요한 할 일(IO 변환기 실양식 맞추기·import 테스트 등)은 **회사 PC 세션**에서. **맥북 세션**은 계획·문서·도구 코드(.ps1 로직, .xlsx 직접 읽기 등) 다듬기 중심.

> 참고: 위 6·8번의 옛 "다음 세션" 목록은 과거 기록. **최신 진행 = 바로 위 2026-06-18 항목 + 10번 "▶ 다음 세션".**
